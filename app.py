...
from PIL import Image # Image Support
...
# ---------------------- FEATURE MENU (Expanded to 8 Columns) ----------------------
...
if col_bt3.button("🖼️ To Image"): st.session_state.mode = "ToImage"
if col_bt4.button("📝 To Text"): st.session_state.mode = "ToText"
...
# Multi-functional file uploader based on mode
if st.session_state.mode == "Merge":
    uploaded_files = st.file_uploader("Upload Multiple PDFs to Merge", type=["pdf"], accept_multiple_files=True)
elif st.session_state.mode == "ImgToPdf":
    uploaded_files = st.file_uploader("Upload JPEGs/PNGs to make PDF", type=["jpg", "jpeg", "png"], accept_multiple_files=True)
else:
...
# 1. MERGE PDFs
if st.session_state.mode == "Merge" and uploaded_files:
    if st.button("🚀 Merge Now"):
        ...
        out = io.BytesIO()
        result_pdf.save(out)
        st.success("All PDFs Merged!")
        open_pdf_in_new_tab(out.getvalue())

# 2. IMAGE TO PDF (JPEG ADD)
elif st.session_state.mode == "ImgToPdf" and uploaded_files:
    if st.button("🚀 Convert to PDF"):
        img_doc = fitz.open()
        for img_file in uploaded_files:
            img = Image.open(img_file)
            pdf_bytes_temp = img_file.getvalue()
            img_pdf_bytes = img_doc.convert_to_pdf(pdf_bytes_temp)
            img_doc.insert_pdf(fitz.open("pdf", img_pdf_bytes))
        out_img = io.BytesIO()
        img_doc.save(out_img)
        st.success("Images converted to PDF!")
        open_pdf_in_new_tab(out_img.getvalue())

# 3. SPLIT PDF
elif st.session_state.mode == "Split" and uploaded_file:
    doc_split = fitz.open(stream=uploaded_file.read(), filetype="pdf")
    page_num = st.number_input(f"Enter Page to Extract (1-{len(doc_split)})", 1, len(doc_split), 1)
    if st.button("🚀 Split & Save"):
        new_doc = fitz.open()
        new_doc.insert_pdf(doc_split, from_page=page_num-1, to_page=page_num-1)
        out_sp = io.BytesIO()
        new_doc.save(out_sp)
        open_pdf_in_new_tab(out_sp.getvalue())

# 4. ORIGINAL CORE LOGIC (Smart OCR, Replace All, Image, Text, Editor)
elif uploaded_file:
    ...
    if st.session_state.mode == "OCR":
        if st.button("🔍 Start Deep OCR Scan"):
            with st.spinner("Scanning images..."):
                try:
                    ...
                    full_ocr = "".join([f"Page {i+1}\n{pytesseract.image_to_string(img)}\n" for i, img in enumerate(images)])
                    st.text_area("OCR Text", full_ocr, height=300)
                except Exception as e: st.error(f"Error: {e}")

    elif st.session_state.mode == "ReplaceAll":
        f_all = st.text_input("Find"); r_all = st.text_input("Replace")
        ...
        for i in range(len(doc_img)): st.image(doc_img[i].get_pixmap().tobytes("png"), caption=f"Page {i+1}")

    elif st.session_state.mode == "ToText":
        doc_txt = fitz.open(stream=pdf_bytes, filetype="pdf")
        st.text_area("Text Content", "".join([p.get_text() for p in doc_txt]), height=300)
        ...
        with st.container():
            ...
            bg_color = c4.color_picker("Background Patch", "#FFFFFF") 
            ...
            if st.button("✨ Apply Smart Transformation"):
                ...
                for page in doc_edit:
                    for rect in page.search_for(find_txt):
                        ...
                        if is_underline: page.draw_line(fitz.Point(rect.x0, rect.y1), fitz.Point(rect.x1, rect.y1), color=tuple(int(t_color.lstrip('#')[i:i+2], 16)/255 for i in (0, 2, 4)), width=1)
                out_ed = io.BytesIO(); doc_edit.save(out_ed); open_pdf_in_new_tab(out_ed.getvalue())

    # ---------------------- ORIGINAL VIEWER & ANALYZER (UNTOUCHED) ----------------------
    st.divider()
