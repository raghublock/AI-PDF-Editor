...
# ---------------------------------------------------------
# ENHANCED CSS (SCROLLBAR & WHITE SCREEN FIX ADDED)
# ---------------------------------------------------------
st.markdown("""
<style>
    /* Global Styles */
    ...
    /* 1. BUTTON LINE SCROLLBAR (Horizontal Scroll for Tabs) */
    .stTabs [data-baseweb="tab-list"] {
    ...
    /* 2. WHITE SCREEN FIX (Text Visibility) */
    .main-card { 
        ...
        margin-bottom: 20px;
        transition: transform 0.3s ease;
        color: #1f1f1f !important; /* Force text to show on white card */
    }
...
st.markdown("""
    <div style='text-align: center; padding: 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
        ...
        <p style='color: white; opacity: 0.9; margin: 10px 0 0 0;'>🚀 Advanced PDF Processing Tool with AI Features</p>
    </div>
""", unsafe_allow_html=True)

# (Session State & Universal Functions - Code remain exactly same as yours)
def init_session_state():
    defaults = {'viewer_pdf_bytes': None, 'viewer_doc': None, 'processed_pdfs': {}, 'cache_cleanup_time': time.time()}
    for key, value in defaults.items():
...
def get_deep_page_colors(page):
    colors = set(); text_data = []
    blocks = page.get_text("dict")["blocks"]
    for b in blocks:
        if b['type'] == 0:
            for l in b["lines"]:
                for s in l["spans"]:
                    hex_c = "#{:02x}{:02x}{:02x}".format((s["color"]>>16)&255,(s["color"]>>8)&255,s["color"]&255)
                    colors.add(hex_c); text_data.append({"Text": s["text"], "Font": s["font"], "Size": s["size"], "Color": hex_c})
    return colors, text_data

# ---------------------------------------------------------
# TAB SETUP
# ---------------------------------------------------------
tabs = st.tabs(["📘 Viewer + Edit", "➕ Merge", "✂ Split", "🗜 Compress", "🖼 Images", "📄 Text", "📊 Tables", "📑 Reorder", "✍ Sign", "🤖 AI Edit"])

# =========================================================
# 📘 TAB 1 — VIEWER + SMART EDITOR (FIXED FOR LARGE PDF)
# =========================================================
with tabs[0]:
    st.markdown("<div class='main-card'>", unsafe_allow_html=True)
    uploaded_file = st.file_uploader("📂 Upload PDF", type=["pdf"], key="v_up")

    if uploaded_file:
        pdf_bytes = uploaded_file.read()
        doc = fitz.open(stream=pdf_bytes, filetype="pdf")
        
        # UI controls (Preserving your original logic)
        col1, col2 = st.columns(2)
        find_txt = col1.text_input("🔍 Find Text")
        replace_txt = col2.text_input("✏️ Replace With")
        ...
        if st.button("✨ Apply Smart Replace", use_container_width=True):
            ...
            for page in doc_ed:
                for rect in page.search_for(find_txt):
                    ...
                    page.insert_text(fitz.Point(rect.x0, rect.y1-1), replace_txt, fontsize=10, fontname="helv", color=(0,0,0))
            out = io.BytesIO(); doc_ed.save(out)
            st.success("🎉 Edit Complete!"); open_pdf_in_new_tab(out.getvalue()); add_whatsapp_share()

        st.divider()
        
        # 3. LARGE PDF PREVIEW FIX (>2MB SUPPORT)
        st.subheader("📄 PDF Smart Preview (Large File Support)")
        zoom = st.slider("🔍 Zoom Preview", 50, 200, 100, key="z_s")
        with st.spinner("Rendering High-Quality Preview..."):
            # fitz pixmap Rendering bypasses the 2MB data-URI limit
            for i in range(len(doc)):
                page = doc[i]
                pix = page.get_pixmap(scale=zoom/100)
...
# =========================================================
# ➕ TAB 2 — MERGE (Preserving your full logic)
# =========================================================
with tabs[1]:
    st.markdown("<div class='main-card'>", unsafe_allow_html=True)
    m_files = st.file_uploader("Select PDFs", type=["pdf"], accept_multiple_files=True, key="m_u")
    if m_files and len(m_files) >= 2:
        if st.button("🔗 Merge Now"):
            merger = fitz.open()
            for pdf in m_files:
                with fitz.open(stream=pdf.read(), filetype="pdf") as dt: merger.insert_pdf(dt)
            out = io.BytesIO(); merger.save(out)
            st.success("🎉 Merged!"); open_pdf_in_new_tab(out.getvalue()); add_whatsapp_share("Merge ho gayi!")
    st.markdown("</div>", unsafe_allow_html=True)

# (Baki saare 1000 lines ka logic aapka iske niche continue rahega...)
